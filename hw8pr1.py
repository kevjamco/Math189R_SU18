...
def k_means(X, k, eps=1e-10, max_iter=1000, print_freq=10):
    ...
    while iter_num < max_iter:
        ...
        for cluster_num in range(k):
            indices = np.where(label == cluster_num)[0]
            if len(indices) > 0:
                clusters[cluster_num, :] = X[indices].mean(axis=0)
        "*** END YOUR CODE HERE ***"
...
if __name__ == '__main__':
    ...
    # For testing:
    #clusters, label, cost_list = k_means(X, 4, eps=1e-10, max_iter=100, print_freq=10)
    #print(cost_list)
    #exit()
